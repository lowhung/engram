...
impl AdjacencyGraph {
    ...
    /// where each node typically talks to only 1-2 other nodes.
    pub fn sample(seed: u64) -> Self {
        ...
        // Create nodes with simple names
        for i in 0..node_count {
            all_nodes.push(GraphNode {
                ...
                throughput: rng.gen_range(1000..50000),
            });
        }

        // Create sparse connections: each node writes to 1 topic, reads from 0-1 topics
        // This creates a simple directed graph with ~1-1.5 edges per node
        for i in 0..node_count {
            let topic = format!("topic_{}", i);
            all_nodes[i].writes.push(topic.clone());
            producers
                .entry(topic.clone())
                .or_default()
                .push(all_nodes[i].name.clone());

            // Pick 1-2 random consumers for this topic (not self)
            let consumer_count = rng.gen_range(1..=2);
            let mut available: Vec<usize> = (0..node_count).filter(|&j| j != i).collect();
            available.shuffle(&mut rng);

            for &consumer_idx in available.iter().take(consumer_count) {
                all_nodes[consumer_idx].reads.push(topic.clone());
                consumers
                    .entry(topic.clone())
                    .or_default()
                    .push(all_nodes[consumer_idx].name.clone());
            }
        }

        // Build edges from producer->consumer relationships
                    ...
                    for consumer in topic_consumers {
                        if producer != consumer {
                            edges.push(GraphEdge {
                                ...
                                target: consumer.clone(),
                                topic: topic.clone(),
                                rate: Some(rng.gen_range(10.0..100.0)),
                            });
                        }
                    }
                }
